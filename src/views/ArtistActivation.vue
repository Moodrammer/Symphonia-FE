<template>
    <div class="mainContainer">
        <v-row justify="center" align-content="center" v-if="isLoading" class="centering">
            <pulse-loader :loading="isLoading" color="black" size="20px"></pulse-loader>
        </v-row>
        <v-container fluid  v-if="!isLoading" >
        <symphonia-header v-if="!isLoading"></symphonia-header>
        <v-divider></v-divider>
            <v-content style="max-width: 600px; margin: auto" class="my-5">
                    <v-row v-if="isActivationTokenValid" justify="center" align-content="center" cols="12">
                        <h1 style="text-align: center;">Congratulations 🎉!</h1>
                        <h2 style="text-align: center;">You have successfully joined the family of Symphonia artists</h2>
                        <div style="text-align: center">
                            you can now customize your profile, upload albums and songs and let the world hear you
                        </div>
                    ...
                    <v-row v-if="!isActivationTokenValid" justify="center">
                        <v-col v-if="isActivationfailed">
                            <h1 style="text-align: center;">The activation link you provided is invalid</h1>
                            <h2 style="text-align: center;">Please re-check your email for the sent link and try again</h2>
                        </v-col>
                        <v-col v-if="isServerError">
                            <h1 style="text-align: center">We're currently having some issues, Please try again later</h1>
                        </v-col>
                    </v-row>
            </v-content>
        </v-container>
...
<script>
    ...
    export default {
        ...
        async mounted() {
            try{
                let response = await axios.patch(`/v1/users/activate/${this.artistActivationToken}`);
