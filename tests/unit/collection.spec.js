...
collection.forEach(collectionElement => {
  ...
  it("menuOrder function", () =>{
    ...
    let menuItem, cardIndex = 4;
    if(collectionElement === artists)
      menuItem = "Unfollow"
    else
      menuItem = "Remove from your Library"
  ...
  if(collectionElement === playlists)
  {
    ...
    it("tracksGetter watcher", async() =>{
      ...
      await wrapper.vm.$nextTick()
      expect(wrapper.vm.cardItems.likedSongs).toEqual([-1,20032,323,-5,11,7,-8,9,232]);
    });
  ...
  }else if(collectionElement === albums)
  {
    it("allAlbums watcher", async() =>{
      ...
      await wrapper.vm.$nextTick()
      expect(wrapper.vm.cardItems.items).toEqual([92341,2,3,50,6932,437,8,3219,13210]);
    });

  }else{
    it("allFollowedArtists watcher", async() =>{
      store.state.artist.followed = [3241,45352,390,3215,986,2217,823,4329,1230]
      console.log(store.state.artist)
      await wrapper.vm.$nextTick()
      expect(wrapper.vm.cardItems.items).toEqual([3241,45352,390,3215,986,2217,823,4329,1230]);
    });
